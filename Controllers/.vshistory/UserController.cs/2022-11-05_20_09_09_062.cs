    ...
    public class UserController : ControllerBase
    {
        ...
        private async Task<string> WriteFile(int id, IFormFile file, [FromForm]ContactPerson contactPerson)
        {
            ...
            try
            {
                ...
                await User.UpdateUser(id, contactPerson);
                db.SaveChanges();
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> AddNewItem(int id, IFormFile file, [FromForm] ContactPerson contactPerson, CancellationToken cancellationToken)
        {
            var result = await WriteFile(id,file, contactPerson);
